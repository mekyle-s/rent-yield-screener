...
The build will run partly unattended (autonomy ladder Rung 2+). Whatever a machine can't verify silently rots — so every ROADMAP task carries executable acceptance criteria, and the test pyramid must make those criteria cheap to assert. Constitution VI additionally makes ETL determinism a *testable law*.
