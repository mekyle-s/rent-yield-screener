...
**PHASE C: C.0 ✅ (ea06aee, CI green) · C.1 ✅ (b3ac3d9) · C.2a ✅ (60c3823) · C.2 ✅ (this commit) · C.4 ✅ (top-50 curated) · C.3 next (Rung-3 candidate).** Phase INFRA record (T1–T9, hooks 110→216 matrix, Rung-3 proven): `decisions/0005-autonomy-infra.md` + git history of this file.
...
2026-07-13: **C.2 Metro page template DONE (Rung-3 loop iteration), all four ACs green with output shown in-session:**

1. `src/pages/metro/[slug].astro` — one page per fixture metro via `getStaticPaths()` over `latest.metros`, slug from the existing `src/lib/slug.ts` (reused from C.1, index.astro hrefs already pointed here). Astro default directory output → `dist/metro/<slug>/index.html`. Shows ratio, ZHVI, ZORI (via new `src/lib/format.ts` deterministic comma formatter — no `Intl`/`toLocaleString`, matches repo's zero-Intl convention), snapshot month, state, a cross-sectional rank ("N of 15 fixture metros by ratio"), back-link to `/`, and the exact attribution string in a `<footer>`.
2. **Scope note (assumption stated per non-negotiable #2):** ROADMAP's C.2 prose also mentions a "12-mo trend sparkline" and a "top-ZIP table (fixture zips joined by metro)". Neither is achievable from current data without an ETL/schema change: `latest.json` records only the single latest shared month per region (B.2's transform intentionally discards history — no monthly series survives), and there is no reliable zip→metro join key (`data/crosswalk/metro-cbsa.csv` maps metro RegionID→CBSA only; the raw ZIP CSVs carry a `Metro` column but it holds full CBSA names — e.g. "Houston-The Woodlands-Sugar Land, TX" — that do NOT string-match the metro-level `RegionName` used elsewhere — e.g. "Houston, TX" — so a safe join would need a name crosswalk that doesn't exist). Both features are outside the 4 checked ACs (neither AC references a sparkline or ZIP table). Building either would mean either fabricating data or reopening Phase B's already-golden-tested transform — out of scope for one task iteration. Implemented honest cross-sectional content instead (rank among the 15 fixture metros) using only real, already-joined data. Flagging for a human call on whether to scope a follow-up task (e.g. add `Metro` passthrough + a name crosswalk in a future ETL change) rather than guessing.
3. Test-first: `tests/format.test.ts` (5 cases, RED confirmed before `src/lib/format.ts` existed) and `tests/slug.test.ts` (3 cases, incl. a slug-uniqueness guard over the golden snapshot's 15 metro names — a silent collision there would quietly drop a page and break AC1 without any error).
4. AC proof (Git Bash):
   - `npm run build && find dist/metro -name "*.html" | wc -l` → `15`
   - `grep -rL "Data Provided by Zillow Group" --include="*.html" dist/metro | wc -l` → `0`
   - `grep -o 'data-snapshot-month="2026-05"' dist/index.html | wc -l` → `1` (index.astro untouched by this task, so the C.1-inlined count stays exactly 1; metro pages carry no `data-snapshot-month` attribute at all, by design, so there's no risk of a second occurrence anywhere)
   - `npm test; echo "exit=$?"` → 224/224 passed (216 prior + 8 new), `exit=0`
   - Also verified (not part of the C.2 AC list but CI-relevant): `npm run check` → 0 errors/0 warnings; `npx prettier --check .` → clean.

2026-07-12: **C.4 DECISION: 50 — top-50 curated metro build (advisor gate 2026-07-12, verdict confirmed).** Lower-bound Ahrefs Starter figures against the pre-registered basket (54 rows = 18 head + 36 city, reconciled exactly): head-term aggregate **4,650/mo** (falls in the 1.5K–5K curated band; full-build trigger fails both conjuncts) · city long-tail avg **0.83/mo** per variant (30 ÷ 36; only nyc/chicago/los angeles non-zero at 10 each). Boundary flag (4,650 within ±20% of the 5K boundary) resolved per the 2026-07-12 advisor ruling — ambiguity escalation retired (its target became the instrument itself); Ahrefs numbers bind, no third measurement. **Consequence: D.1 scales to 50 curated metros, not 400. C.1–C.3 scope unchanged (15 fixture metros this phase).** Recorded in ROADMAP § Phase C and HQ `01_idea_selection/02-decision.md`.
...
**C.2 ✅ COMPLETE 2026-07-13 — this iteration halts here per the loop task's own instruction (Task C.2 scope only; do not start C.3).** Human decision needed before C.3/D.1 planning: whether to scope a follow-up task adding raw-ZIP `Metro`-column passthrough + a name crosswalk (no such crosswalk exists today) so a real "top-ZIP table" and multi-month trend become possible — see the scope note under 2026-07-13 in Last action & result. NEXT (for a future iteration, NOT this one): **C.3 — SEO plumbing: site artifacts + checker, NO CI edit**, Rung-3 loop candidate, exact ACs in ROADMAP § Phase C. After C.3: C.3b (ATTENDED CI edit, one YAML line). C.4 ✅ COMPLETE 2026-07-12 — top-50 curated verdict recorded; D.1 scales to 50 curated metros. Findings 13,15–18,20,21 remain queued for Phase 5.
...
**2026-07-13 — LOOP:HALT reason (this iteration's commit):** C.2's tasked scope is complete — all 4 ACs green, no eligible Next action remains for THIS task. Halting per the iteration contract's "one task per iteration" rule, not because of a blocker: C.3 is a separate eligible Rung-3 candidate and may start in a fresh iteration. Flagged (not blocking) for a human look before C.3/D.1 planning: whether a follow-up task should extend the ETL to carry the ZIP-level `Metro` column + a name crosswalk, enabling a real top-ZIP table and multi-month trend on metro pages (currently infeasible from `latest.json` alone — see the C.2 scope note in Last action & result).
