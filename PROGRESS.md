...
**T6 — loop credentials (SUPERVISED — needs Mekyle):** (1) create fine-grained PAT at github.com/settings/personal-access-tokens: repo `mekyle-s/rent-yield-screener` ONLY, permissions Contents R/W + Pull requests R/W + Metadata R, 30-day expiry, NO `workflow` permission; (2) run `claude setup-token`; (3) both land in `~/.claude-loop.env` as `GH_TOKEN=` / `CLAUDE_CODE_OAUTH_TOKEN=` (never committed; global git ignore covers nothing here — file lives OUTSIDE the repo). AC: in-container `gh api repos/mekyle-s/rent-yield-screener` → 200; `claude -p "reply OK"` → OK; clone+push scratch branch. Then T7 (loop.sh + PROMPT.md), T7.5 (scoped review gate, scope estimate + approval first), T8 (dry-run), T9 (close-out).
