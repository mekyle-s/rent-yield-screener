...
export type ValidationResult =
  { ok: true } | { ok: false; token: string; detail: string };

const fail = (token: string, detail: string): ValidationResult => ({
  ok: false,
  token,
  detail,
});
...
export function validateCsvSchema(text: string): ValidationResult {
  ...
  if (!firstLine)
    return fail("SCHEMA_VIOLATION:", "empty file or missing header row");
  ...
  if (dateCols.length === 0)
    return fail("SCHEMA_VIOLATION:", "no date columns in header");
  ...
  const overlap = (expected: string[]) =>
    metaCols.filter((c) => expected.includes(c)).length;
  const expected =
    overlap(ZIP_META) > overlap(METRO_META) ? ZIP_META : METRO_META;
  for (const col of expected)
    if (!metaCols.includes(col))
      return fail("SCHEMA_VIOLATION:", `missing meta column ${col}`);
  for (const col of metaCols)
    if (!expected.includes(col))
      return fail("SCHEMA_VIOLATION:", `unexpected meta column ${col}`);
  if (metaCols.join(",") !== expected.join(","))
    return fail(
      "SCHEMA_VIOLATION:",
      `meta column order changed: ${metaCols.join(",")}`,
    );
...
export function validateLatest(
  ...
  if (doc.metros.length < opts.minMetros)
    return fail(
      "ROWCOUNT_ANOMALY:",
      `metros=${doc.metros.length} < min ${opts.minMetros}`,
    );
  if (doc.zips.length < opts.minZips)
    return fail(
      "ROWCOUNT_ANOMALY:",
      `zips=${doc.zips.length} < min ${opts.minZips}`,
    );
  ...
  for (const r of doc.metros)
    if (!Number.isFinite(r.ratio) || r.ratio < lo || r.ratio > hi)
      return fail(
        "RATIO_RANGE:",
        `metro ${r.regionId} ratio=${r.ratio} outside [${lo},${hi}]`,
      );
  ...
  for (const r of doc.zips)
    if (!Number.isFinite(r.ratio) || r.ratio <= 0)
      return fail(
        "RATIO_RANGE:",
        `zip ${r.regionId} ratio=${r.ratio} non-positive or non-finite`,
      );
  ...
  if (doc.zips.length > 0) {
    ...
    if (zipMedian < lo || zipMedian > hi)
      return fail(
        "RATIO_RANGE:",
        `zip median=${zipMedian} outside [${lo},${hi}] — systematic corruption?`,
      );
  }
