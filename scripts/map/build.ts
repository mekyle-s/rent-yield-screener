...
const binClass = (ratio: number) =>
  `p2r-b${BINS.filter((t) => ratio >= t).length + 1}`;
...
const boundaries = JSON.parse(
  readFileSync("data/boundaries/cbsa.geojson", "utf8"),
);

const featureByGeoid = new Map<string, GeoJSON.Feature>(
  boundaries.features.map((f: GeoJSON.Feature) => [
...
for (const m of latest.metros) {
  ...
  if (!d) {
    unmatched.push(
      `${m.regionId} (${m.regionName}) -> ${geoid ?? "NO CROSSWALK ENTRY"}`,
    );
    continue;
  }
...
if (unmatched.length > 0) {
  process.stderr.write(
    `MAP_BUILD: ${unmatched.length} metro(s) failed the boundary join:\n`,
  );
...
console.log(
  `${OUT}: ${paths.length}/${latest.metros.length} metro paths (${fixtures ? "fixtures" : "live"})`,
);
